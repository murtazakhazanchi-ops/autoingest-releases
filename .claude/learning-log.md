...
### 2026-08-07 — Part 8: Multi-AI Engineering Conversation Integration — Parallel Security/Performance/Code Review Findings

Task type:
- New Feature (Documentation-Tooling Subsystem) / Parallel Review (code-reviewer + performance-auditor + security-reviewer) / Security Hardening

What happened:
- Part 8 added "Multi-AI Engineering Conversation Integration" to AutoIngest's `docs/product/` documentation-intelligence tooling (`scripts/product-docs/`) — an import pipeline (`scripts/product-docs/automation/conversation/`) that ingests external engineering discussions (ChatGPT, Codex, Gemini, human meetings) into a new `ENG-CONV-####` canonical record family. Three review agents ran in parallel against the full diff and found 8 real, fixed issues. This subsystem is outside the Electron app (no `event.json`, no IPC, no renderer) — a separate Node.js CLI tool suite — but several findings are general software-correctness/security patterns durable enough to generalize to future AutoIngest work, not just this one pipeline.
- **Sanitize-once at trust boundary**: `scripts/product-docs/automation/conversation/compiler.js` interpolated packet free-text fields directly into compiled Markdown with no escaping. `scripts/product-docs/lib/markdown.js`'s `extractHeadings()`/`extractSection()` treat any line matching `^#{1,6}\s+(.+)$` as a real section boundary anywhere in the document. Security review proved a packet whose `user_goal` embedded a forged `## Outcome` line produced a compiled record where `extractSection(content, 'Outcome')` returned the forged text instead of the real one — and `hookAutomation.js`'s `linkCommitToConversations` (which greps the Outcome section to decide whether to skip reconciliation) would be fooled by it, permanently hiding real implementation status. Fix: new `markdownSanitizer.js` applying `sanitizeDeep()` exactly once, in `lifecycle.js#analyzePacket` immediately after redaction, before any renderer/parser (compiler.js, decisionLink.js, memoryLink.js) sees the packet.
- **Raw untrusted claim used to gate a canonical write, instead of the already-validated result**: `decisionLink.js`'s `hasSufficientEvidence()` originally read `packet.related_feature_ids`/`packet.related_roadmap_ids` (the packet's own raw claim) instead of `ownership.primary_feature_ids`/`ownership.roadmap_ids`, which `ownership.js`'s `resolveConversationOwnership()` had already computed earlier in the same pipeline by filtering through `validExistingIds()`. A packet claiming a nonexistent feature ID would still clear the evidence bar and get a canonical `DEC-###` drafted citing it — a permanent, non-reusable ID spent on a broken record. Fix: `linkOrDraftDecision` now requires the caller to pass the validated `ownership` object; `hasSufficientEvidence` never reads the raw packet fields.
- **Permanent-ID allocation lifecycle**: (a) gate — `significance.js`'s `planConversationCanonicalization()`, modeled on the pre-existing `automation/memory/significance.js`'s `planMemoryCapsule()`, gates `ENG-CONV-####` allocation on explicit content markers (decisions, revisions, bugs, requirements, etc.) actually being present, catching a naive design (architecture review, before code was written) that would have burned permanent IDs on every schema-valid, non-duplicate import regardless of content. (b) isolate — `lifecycle.js#importPacket` allocates the real `ENG-CONV-####` and writes a first-pass canonical file *before* running decision/bug/memory linkage. Code review found the three linkage calls originally shared one outer try/catch: if any threw, the whole import was reported `outcome: 'failed'` even though the canonical file — already written with linkage fields rendered `null` — was never rolled back or flagged, leaving a permanent, silently-incomplete record while the audit trail claimed total failure. Fix: each of the three linkage calls (`linkOrDraftDecision`, `linkBugs`, `linkOrCreateMemory`) now wrapped in its own try/catch, degrading to an `{ state: 'error', error }` result instead of aborting the already-ID-spent import.
- **Untrusted-content pipeline structurally isolated from auto-finalize automation**: architecture review found that reusing `decisionIntelligence.js`'s `scanPacket()` directly against a live Evidence Packet session would let untrusted external conversation text enter the same stream `hookAutomation.js`'s pre-push auto-finalize already consumes — letting it shape a canonical decision automatically via keyword matching, with no human in the loop, via nothing more than `git commit`/`git push`. Fix: `decisionLink.js`/`bugLink.js`/`memoryLink.js` are only ever called from `lifecycle.js#importPacket` (an explicit CLI action); `hookAutomation.js`'s `postCommitLink`/`prePushGate` never call into any of them — `postCommitLink`'s `linkCommitToConversations` only reads existing canonical records and writes an Outcome update from real commit evidence, never imports anything new.
- **Substring containment used for identity/dedup matching**: `dedupe.js`'s `findExactDuplicate()` originally did `provenance.includes(String(packet.source_conversation_id))` — a substring check against the whole Provenance section, which also contains boilerplate ("adapter", format names, etc.). Security review proved a packet with `source_conversation_id: "adapter"` matched an unrelated existing record purely because the Provenance section's "Transformation method: ecp adapter (...)" boilerplate line contained the substring "adapter," with a non-matching fingerprint — silently misclassifying a genuinely distinct conversation as an exact duplicate. Fix: `extractSourceConversationId()`, a strict per-line regex (`/^-\s+\*\*Source conversation metadata\*\*:\s*(.+)$/m`) extracting the exact bullet value, then exact string equality.
- **Untrusted data interpolated into `String.replace()`'s replacement-string argument**: `conversationCli.js`'s `cmdLink` did `content.replace(re, \`$1 ${merged} $3\`)`, embedding a CLI-supplied ID value into the replacement pattern *string*. Security review reproduced that a value containing `$'` caused `replace()` to splice in text following the match, corrupting the target record's Markdown table row (replacement strings specially interpret `$&`, `` $` ``, `$'`, `$$`, `$<n>`). Fix: `content.replace(re, (matched, g1, g2, g3) => \`${g1} ${merged} ${g3}\`)` — a replacer function's return value is never re-interpreted for `$`-sequences.
- **Expensive parse paid unconditionally in a hot/frequently-triggered path**: performance review found `hookAutomation.js`'s `postCommitLink` called `build.assemble()` (a full `docs/product/` parse) unconditionally on every commit, even when zero relevant files changed or `docs/product/conversations/` didn't exist yet — because the existence/emptiness check lived inside `linkCommitToConversations`, called only after `build.assemble()` had already run in the caller. This reintroduced a bug class the codebase's own comment on `runValidate` says was "found in Part 7 performance review" and already fixed once for a different caller. Fix: extracted `conversationLinkingCouldApply(changed)` — checking `changed.length > 0` and the conversations directory's existence — called by `postCommitLink` *before* `build.assemble()`.

Reusable lessons:
1. Sanitize untrusted external free-text exactly once, centrally, at the point it enters the pipeline — before any downstream renderer/parser that treats structural syntax (Markdown headings, etc.) as meaningful. Forged structural syntax in unescaped untrusted text is a deterministic parser-integrity bug, not just a prompt-injection concern.
2. Once a validation/resolution step has computed a trustworthy result earlier in a pipeline, every downstream consumer that gates a canonical write must consume that computed result — never the raw, unvalidated input field that seeded it. This is a sibling to the existing "Equivalent-Evidence-Across-Branches" resolver rule, but the failure mode is different: not mismatched evidence across branches, but bypassing already-done validation entirely.
3. Permanent/non-reusable identifier allocation needs two-sided discipline: gate allocation behind an explicit significance/content predicate *before* allocating (not implicit via dedup-only checks), and once allocated, isolate every subsequent side-effect operation in its own try/catch so one failure degrades to an honest partial result instead of an all-or-nothing collapse that leaves an orphaned, silently-incomplete artifact while the audit trail claims total failure.
4. Any pipeline whose input includes untrusted/external content must be structurally reachable only through an explicit, foreground, human-invoked action — never through an automatic finalize/merge/git-hook trigger. Reusing an existing scanner/matcher against untrusted content inside an already-automatic pipeline silently extends that automation's trust boundary.
5. Identity/dedup matching against an extracted provenance or metadata field must use exact-value comparison against a precisely, narrowly extracted field (strict per-line regex or equivalent) — never substring/`.includes()` search across a whole rendered prose section that may also contain shared boilerplate text.
6. Never interpolate untrusted/dynamic values into the replacement-*string* argument of `String.prototype.replace()` — use a replacer *function*. A replacement string specially interprets `$&`, `` $` ``, `$'`, `$$`, `$<n>` sequences that untrusted input could contain; a replacer function's return value is never re-interpreted this way.
7. In a hot/frequently-triggered path (git hook, per-request handler), a cheap existence/relevance pre-check must run *before* the expensive operation it guards — not be embedded inside the expensive operation's own callee, where the expensive work has already happened by the time the cheap check runs. This exact bug class recurred here after already being fixed once for a different caller in the same file (per the file's own Part 7 comment) — a strong signal it is worth capturing durably rather than re-discovering per caller.

Not promoted as separate rules:
- Lessons 3's two halves (significance-gating before allocation, try/catch isolation after allocation) were merged into a single consolidated "Permanent ID Allocation Lifecycle" rule rather than two separate agent rules, to avoid two near-duplicate entries in an already-long agent file — they are two sides of the same allocation lifecycle, not independent patterns.
- No candidate from this review was rejected outright; all 8 met the durability/reusability bar (per `.claude/learning-rules.md`), but two were consolidated into one rule during promotion (see above) rather than kept as 1:1 copies.

Promote to agents:
- `autoingest-architect.md` — lessons 1, 2, 3, 4 as new Learned Rules. These are general architecture/security-boundary patterns (trust-boundary sanitization, validated-result consumption, permanent-ID lifecycle discipline, untrusted-content pipeline isolation) consistent with the agent's existing generically-worded rules (Equivalent-Evidence-Across-Branches, Read-Only Aggregation Service Pattern) even though sourced from a non-Electron subsystem.
- `code-reviewer.md` — lessons 5, 6 as new checklist-style Learned Rules (common, greppable bug classes: substring-vs-exact identity matching, `.replace()` replacer-function requirement).
- `performance-auditor.md` — lesson 7 as a new Learned Rule, directly extending the existing "Bottleneck Classification First" ethos with a concrete ordering rule (cheap check before expensive work in hot paths).
- Not touched: `event-data-guardian.md`, `contract-debugger.md`, `ui-system-specialist.md`, `metadata-specialist.md`, `ingestion-routing-specialist.md`, `group-mapping-specialist.md`, `documentation-update-specialist.md`, `release-docs-writer.md` — all are scoped to Electron-app-specific or human-doc-writing domains; none owns `scripts/product-docs/automation/` and none of the 7 promoted lessons is specific to their domains.
...
### 2026-08-03 — Audit & Repair Tab Layout Rebalance (Metadata Management)

Task type:
- UI Layout Rebalance / Multi-Round Plan Hardening / CSS Root-Cause Debugging

What happened:
- Rebalanced the Audit & Repair tab (`#msTabAudit`) layout inside the Metadata Management modal so the results zone, footer, and sibling tab-bar/panels kept consistent spacing after a redesign. The plan went through 6 rounds of `ExitPlanMode` revision before approval — the user repeatedly pushed back on drafts that described the visual hierarchy without specifying the actual space-distribution mechanism (which element grows, which stays fixed, and how the parent enforces that).
- Implementation surfaced 2 real bugs during automated testing, both CSS root-cause issues rather than typos:
  1. `height: 100%` on the active tab panel silently overflowed its parent (`#msBody`) by exactly the height of the sibling tab bar (`.ms-tabs`), because percentage height resolves against the parent's total content box, not "space left after visible siblings." Caught only because an automated test asserted the results-zone-to-footer gap numerically, not by eyeballing a screenshot. Fixed by converting `#msBody` to `display:flex; flex-direction:column`, giving `.ms-tabs` and the two inactive tab panels `flex-shrink:0` to preserve their exact prior sizing, and changing the active panel to `flex:1` instead of `height:100%`.
  2. A scoped override (`#msTabAudit #maList { display:flex; flex-direction:column; gap:4px; }`) intended to cancel the base class `.diag-list`'s `flex:1` and `overflow-y:auto` did not actually cancel them — CSS cascade only overrides properties the more-specific rule explicitly declares; properties it omits still resolve from the highest-specificity rule that does declare them. This produced a nested double-scrollbar and broke an unrelated centering technique that depended on the element not being flex-grow. Fixed by explicitly declaring `flex: none; overflow: visible;` in the override.
- Before reusing `.adopt-section` for Repair Preview (which needed a relaxed `max-height`/`overflow`), a grep confirmed the class was also used by the unrelated Orphan File Adoption feature (`#adoptionSection`), depending on the exact same properties. Used a modifier class (`.adopt-section--inline`) instead of editing the base rule.

Reusable lessons:
1. **A child sized to fill remaining space next to a fixed-size sibling needs the parent to be a flex/grid container distributing that space — percentage height (or width) on the child alone cannot account for space consumed by siblings, only for the parent's total size.** General CSS mechanism, not tab-specific: whenever a plan describes "fill the space left after X," verify the parent uses flex/grid before implementing with percentage sizing.
2. **A scoped override only cancels a base class's conflicting properties if it explicitly re-declares them.** Adding new properties in a more-specific selector does not implicitly cancel other properties the base class sets. Verify by diffing computed style, not the written CSS rule, whenever an override is meant to change a shared class's layout behavior (`flex`, `overflow`, `display` in particular).
3. **Grep all usages of a shared class before repurposing it for a new feature's needs, not only before editing its base rule outright or removing it.** If another feature depends on the same properties, add a modifier class instead of touching the base rule.

Not promoted:
- The 6-round plan revision process itself. Prior tasks in this log (Transfer Import: 3 rounds; Dashboard Metadata Health Card: 3 rounds) already show this is a recurring pattern of the user's review style, not a lesson specific to this task — and the underlying discipline ("propose the actual space-distribution mechanism, not just the visual outcome") is captured concretely by lesson 1 above. Round count is an outcome of review depth, not a controllable/verifiable input, so it does not make a good standalone agent rule.

Promote to agents:
- `ui-system-specialist.md` — lessons 1 and 2 as new Learned Rules (general CSS layout/cascade pitfalls; no other agent owns this surface). Lesson 3 as a new rule placed alongside the existing "Remove Dead CSS Rules" / "Entry-Point Removal Audit" grep-before-touching-shared-state family, since it is the same discipline applied to CSS class reuse rather than removal.
...
### 2026-08-03 — Run Audit Full-Width Fix + Obsolete Metadata Audit Entry Point Removal

Task type:
- Surgical UI Bug Fix / Root-Cause CSS Investigation / Dead Entry Point Removal / Test-Suite Hygiene

What happened:
- Commit `6349c62`. Two focused fixes in Metadata Management, done via explicit systematic-debugging root-cause investigation before any change.
- **Run Audit button full-width fix**: `#maRunBtn` already used the correct, unmodified `.emm-btn-primary` class — no width rule of its own. Root cause was the *container*: `.diag-actions` is `flex-direction: column` with no `align-items` override, so flexbox's default `align-items: stretch` stretched every child, including the button, to full container width. Fixed by wrapping `#maRunBtn`/`#maCancelBtn` in a new scoped `.ma-run-row` (`display:flex; gap:8px`), mirroring the existing `.diag-actions-tools` row-wrapping mechanism one element over in the same component, but deliberately not inheriting that sibling class's font-size/padding override. The identical latent bug exists today on Archive Diagnostics' `#diagRunBtn` (same `.diag-actions` class) — deliberately left untouched as out of scope, and explicitly documented as such rather than silently ignored.
- Notably, an earlier, already-promoted agent rule ("Primary Action Button Must Not Be Full-Width in a Modal") had previously fixed this exact area by removing an explicit `.diag-actions #diagRunBtn { width: 100%; }` rule from `#diagRunBtn`. That earlier fix did not actually eliminate the full-width bug — it only removed one contributing cause. The container's default `align-items: stretch` produces the same full-width effect regardless of whether the button has its own width rule, which is exactly why `#diagRunBtn` still exhibits the bug today. This surfaced only because this task's root-cause pass checked the container's flex properties instead of stopping at "the button's class looks correct already."
- **Obsolete entry point removal**: removed `#alocMetadataAuditBtn` (Archive Location Setup → Advanced Archive Operations), a leftover deep-link to Metadata Audit, now reached exclusively via Metadata Management → Audit & Repair since an earlier session's consolidation. Removed markup, click handler, and updated `test/metadataManagementModalUI.test.js`'s corresponding test block to a removal-confirmation check.
- The removal audit's grep of `test/` (not just renderer JS) surfaced two additional, pre-existing problems, neither introduced by this task: (1) `test/metadataPipelineLive.test.js` had been silently broken since the earlier Metadata Management consolidation — it referenced `#metadataAuditModal`, a modal ID that no longer existed at all, never caught because nobody had run that specific test file since. Fixed to use the current entry point (`openMetadataSyncModal({ tab: 'msTabAudit' })` / `#metadataSyncModal`/`#msTabAudit`). (2) Removing the old `#alocMetadataAuditBtn`-click test block in `metadataManagementModalUI.test.js` broke a later, unrelated-looking block — "backdrop click is a no-op while `_metaReapplyPending` is true" — because that block never opened the modal itself; it silently relied on the removed block having left the modal open as a side effect. Fixed by making the backdrop-click block explicitly open the modal itself first, rather than restoring the old accidental coupling.
- Verification was empirical throughout: numerically confirmed rendered button width (97px) vs. container width (678px) via `getBoundingClientRect()`, confirmed zero layout shift across the Run→Cancel state transition by comparing exact coordinates before/after, and used `git stash`/`git stash pop` against the pre-change baseline to confirm an unrelated console CSP warning was pre-existing, not introduced by this fix.

Reusable lessons:
1. **A full-width button bug can exist with zero width rule anywhere on the button or its class — check the parent's flex layout, not just the target element's own class, before concluding a style class is wrong or already fixed.** This is a specific, non-obvious CSS gotcha (not merely "find root cause before fixing," which the debugging process already covers generically): a `flex-direction: column` parent with no `align-items` override stretches children by default regardless of whether the child itself has any width rule. A prior fix that removed an explicit `width: 100%` from a sibling button in the same parent class did NOT eliminate the bug for that sibling — it only removed one contributing cause, and the container-driven stretch independently reproduces the identical symptom. Fix by wrapping the affected button(s) in a small scoped flex row, not by changing the shared parent's `align-items` (which would affect other children that may rely on the current stretch behavior).
2. **Entry-point removal audits must grep `test/`, not only renderer JS and markup.** Renderer JS being clean is not sufficient evidence a removal is complete. A test file that is not part of every routine run can reference a removed/renamed element indefinitely without detection — in this case, a test file had already been silently broken by an *earlier* session's removal, undiscovered until this task's `test/` grep surfaced it. Fix any test-file staleness found this way as part of the current removal pass, even when the staleness predates the current task.
3. **Playwright test blocks that assert on modal/DOM state must establish that state themselves, not rely on a prior unrelated block's side effect.** AutoIngest's Playwright files run as sequential check blocks with no automatic per-block isolation, so this class of hidden coupling is a structural risk, not a one-off: removing or reordering an earlier block can break a later, seemingly-unrelated assertion with a confusing failure. The fix is always to make the later block self-contained (explicitly open the modal / set the precondition itself), never to restore the accidental coupling.

Promote to agents:
- `ui-system-specialist.md` — lesson 1 as an extension of the existing "Primary Action Button Must Not Be Full-Width in a Modal" rule (adds the container-flex diagnostic and the correction that the rule's earlier fix pattern was incomplete). Lesson 2 as a new rule extending the existing grep-for-ID-before-closing-task pattern to explicitly cover `test/`. Lesson 3 as a new rule alongside the existing Playwright-technique rules (contextBridge stubbing, plain-singleton stubbing).
...
### 2026-08-02 — Dashboard Metadata Health Card: Extend-in-Place Plan Correction + Text-Overflow Catch

Task type:
- UI Enhancement / Plan-Review Correction / Playwright Testing / Live-Screenshot Catch

What happened:
- Upgraded the dashboard's `#ovMetadataSync` tile from an external-sync (Bridge/XMP) pending-count-only display into a "Metadata Health" card that shows AutoIngest's own durable per-event metadata state as the primary status, with external-sync status demoted to secondary-line-only context — never the primary label. Pure UI-layer change (`renderer/index.html`, `renderer/renderer.js`, new `test/dashboardMetadataHealthCard.test.js`). No IPC/backend/metadata-logic changes. Both `code-reviewer` and `ui-system-specialist` returned clean approvals on the final diff.
- Plan went through 3 correction rounds, each narrowing scope rather than adding it. Round 1 fixed priority/label design (external-sync must never become the primary label; not-attempted vs not-required needed distinct labels). Round 2 proposed a new shared file (`renderer/metadataHealthMapping.js`, UMD-lite `module.exports` + `window` fallback) to make the priority function unit-testable outside Electron. This was rejected: the user required checking whether the *existing* renderer architecture (5 other separate renderer files — `eventCreator.js`, `eventMgmt.js`, `groupManager.js`, `importRouter.js`, `treeAutocomplete.js`, all `<script src>` IIFE globals) already supported the need. None of them use `module.exports` — so "separate files exist here" did not actually support "unit-testable via `require()`". The correct fix was simpler: extend the existing `METADATA_STATE_LABELS` object in place (3 new fields per entry: `cardLabel`, `cardVariant`, `severity`; existing `label`/`cls` fields untouched) and place the new pure function beside it and beside the existing `_computeMetaStatus()` — zero new files. Round 3 confirmed this and added calm status-color language (blue = default tile appearance, no new CSS; amber = reuse existing `.ov-tile--pending`; red = one new minimal `.ov-tile--error` class, reserved for true failures only; green = text-color tint only via a tiny `.ov-value--ok` utility, not a full tile-variant treatment) plus invariant tests (external status never in `primaryLabel`, `primaryLabel` never empty, `cardVariant` always one of N known values, deterministic output) and a constraint that the helper function must only return presentation data, never do IPC/DOM/CSS work itself.
- A live screenshot pass (not caught by any test or code review) found that the approved full-phrase primary labels ("Metadata Not Yet Applied", "Current Event Complete") wrapped to 2 lines or truncated mid-word ("Current Event Comple…") inside `.ov-value`, an element originally sized for short content (a number, "—"). The first fix attempt was a scoped `#ovMetadataSyncVal { font-size: 0.68rem; }` rule (by element ID, not the shared `.ov-value` class). **This was rejected in a follow-up review round**: the user judged shrinking one tile's font-size as "treating the symptom rather than the UI problem," since it broke typographic consistency across the ~6-tile Overview row even though it was correctly scoped by ID with zero leak to siblings. The actual shipped fix was a redesign, not a smaller version of the same lever: (1) shorter, still self-explanatory `cardLabel` wording ("Metadata Complete", "Attention Required", "Metadata Processing", "Verification Required", "Metadata Queued", "Metadata Pending", "No Metadata Required", "No Event Selected") at the unmodified shared `.ov-value` size (0.92rem; only `line-height: 1.15` was added, for wrapped-line breathing room, not a size change); (2) accepting that some of these legitimately wrap to 2–3 lines at the app's real supported widths — verified, not assumed, via `canvas.measureText()` against every candidate string in a live `page.evaluate()` at 1100px (the app's actual `minWidth` from `main.js`), 1400px (typical), and 1920px (maximized); (3) confirming via `getBoundingClientRect()` on every sibling tile in `#overviewGrid` that all tiles stayed exactly the same computed height as each other at every tested width (e.g. all 6 tiles measured 96px at 1100px, 85px at 1400px, 57px at 1920px) — wrapping is safe here specifically because the flex row's default `align-items: stretch` grows siblings together; the failure mode to avoid was uneven tile heights, not wrapping itself; (4) confirming via screenshots that wrapped content stays fully inside the tile's border box (no clipping) even in the most extreme case (5 wrapped lines total across primary+secondary at 1100px). Both rounds used the identical `canvas.measureText()` technique — what changed between rounds was which lever got pulled in response to the same measurement (font-size vs. wording-and-accept-wrap).
- New Playwright testing technique: to simulate "no active event" / "a specific active event" without navigating the full event-creation UI flow, monkey-patched `EventCreator.getActiveEventData = () => ({...})` directly via `page.evaluate()`. `EventCreator` is a plain in-page JS global (IIFE singleton), NOT behind `contextBridge`, so — unlike `window.api.*`, confirmed unstubbable in an earlier session — it IS freely reassignable from the test. Combined with a real, minimal synthetic `event.json` fixture written to a temp directory so the real `window.api.getMetadataEventState(eventPath)` IPC call genuinely read real fixture data end-to-end.
- Both `code-reviewer` and `ui-system-specialist`, launched in parallel on the first attempt with open-ended review prompts, independently stalled at 600s with no progress and had to be retried. Retries succeeded quickly after prompts were tightened: an explicit `git diff -- <exact file list>` command given verbatim, explicit "do not explore unrelated files / do not re-derive already-confirmed facts / finish within a handful of tool calls" scoping, and pre-supplying facts the first pass had already confirmed. Judged too runtime/session-specific to promote as a durable code-pattern rule — noted here for visibility only, not promoted.

Reusable lessons:
1. **Extend an existing single-owner table/function in place before adding a new shared file — unit-testing convenience is not sufficient justification for a new file.** When a codebase already has multiple "separate module" files, verify they actually share the property that matters (e.g. `module.exports`) before treating their existence as precedent for a new one. Check first whether the existing owning location can simply gain new fields/a sibling pure function.
2. **A live screenshot pass must explicitly check for text-overflow whenever short display content is replaced by longer content — but a scoped local font-size reduction is not the default fix.** Playwright `.textContent` checks and diff-based code review both miss wrapping/truncation because neither inspects rendered pixels; when text doesn't fit, measure real rendered width via `canvas.measureText()` against every candidate string (not just the longest-looking one), at every width the app actually supports (including the real `minWidth`, not just a "typical" width). But when the font-size in question is a shared/global rule used by several visually-equivalent sibling elements (e.g. `.ov-value` across ~6 Overview tiles), a font-size cut scoped to just one instance is a narrower, lower-risk *implementation* than it first appears, yet it still creates an inconsistent typography scale within an otherwise-uniform row — a real design-system violation even when the CSS specificity is correctly scoped and doesn't leak to siblings. Treat that as "treating the symptom": the actual fix is usually content strategy (shorter but still self-explanatory wording) or accepting the layout's natural responsive wrapping — verified safe via `getBoundingClientRect()` height comparison across all siblings at every tested width (not just a screenshot eyeball), confirming wrapping doesn't create uneven tile heights, which is the real failure mode to avoid, not wrapping itself.
3. **Plain in-page JS globals (IIFE singletons assigned to `window`, not routed through `contextBridge.exposeInMainWorld`) are freely stubbable from `page.evaluate()`, unlike `window.api.*`.** Check `main/preload.js` for `contextBridge.exposeInMainWorld` usage before assuming something is unstubbable. Combine a monkey-patched getter with a real minimal fixture file on disk so the real downstream IPC call is exercised end-to-end.

Promote to agents (proposed):
- `ui-system-specialist.md` — lessons 1–3, added as new Learned Rules alongside the existing "Playwright Cannot Stub contextBridge-Exposed APIs" rule (lesson 3, direct companion) and "Live Screenshot Pass Required After UI Restructuring" rule (lesson 2, direct companion). Lesson 1 added near the existing "Merging UI Surfaces With Independent Per-Scope State" rule as a related architecture-decision pattern.
- Agent-dispatch-scoping observation (parallel review agents stalling at 600s on open-ended prompts) — not promoted. Judged as session/runtime-specific rather than a durable code or UI pattern; kept here for visibility only.

Correction (2026-08-02, same task, later review round):
- The font-size-reduction fix described above under lesson 2 was reversed by the user before this task shipped. It was rejected as "treating the symptom rather than the UI problem." Lesson 2 above and the corresponding `ui-system-specialist.md` rule ("Verify Rendered Text Fits When Replacing Short Content With Longer Labels") have been revised in place to reflect the actual shipped fix (content-strategy label wording + verified-safe wrapping) rather than the reverted font-size cut. This note is kept for traceability; do not re-derive the font-size-reduction approach from the "What happened" narrative above without reading it in full — it documents both the rejected attempt and the final approach.
...
### 2026-08-02 — Metadata Management Modal: Consolidating 3 UI Surfaces Into One Tabbed Modal

Task type:
- UI Consolidation / Modal Restructuring / Plan-Review Correction / Playwright Testing

What happened:
- Consolidated three separate metadata-related UI surfaces (`renderer/index.html` + `renderer/renderer.js`) into one "Metadata Management" modal with 3 tabs (Metadata / Audit & Repair / Keyword Registry): the old 2-tab "Metadata Sync" modal (Bridge/XMP review + Keyword Registry), a standalone "Metadata Audit" modal (archive-wide audit scanner + repair), and a duplicated copy of Activity Log's session-scoped "Metadata" filter tab controls (Activity Log's own copy was left in place, not removed). Pure UI restructuring — zero changes to metadata/queue/repair/audit logic, IPC contracts, or ExifTool behavior. Commit `4446a30` on `main`.
- The first plan draft was rejected with 12 substantive corrections (not wording nitpicks): it implied an archive-wide "Apply Metadata" action the backend does not support; it under-specified event-context isolation between the two now-parallel copies of the same controls (risk of `_alCurrentEventPath` bleeding between Activity Log's copy and the new modal's copy, silently acting on the wrong event); it mislabeled a Bridge/XMP pending-sync count as general "Metadata Status"; it proposed entangling audit action-button visibility with the modal's shared footer/tab-switch logic. The revised plan was approved and implemented without further correction cycles.
- Two technical traps surfaced during implementation/testing:
  1. `window.api.someMethod = stubFn` inside Playwright's `page.evaluate()` silently fails against `contextBridge.exposeInMainWorld`-exposed surfaces (non-configurable/read-only). Worked around by asserting on synchronous pre-IPC side effects (pending flag, disabled/relabeled button) of the real click handler instead of intercepting the IPC call. True main-process API mocking (e.g., native dialogs) requires `electronApp.evaluate(({dialog}) => {...})` via `_electron`, not `page.evaluate()`.
  2. `renderer.js` has multiple large top-level IIFEs (e.g. ~line 15109–16664). A function/`let` declared inside one IIFE is not callable from global-scope code elsewhere in the file, even though the IIFE can freely call outer global functions (one-directional lexical scoping). Fixed by explicitly bridging the needed inner function onto `window` right after its definition (`window._maStopPoll = _maStopPoll;`) and calling it as `window._maStopPoll?.()` from outside.
- A live screenshot pass after implementing exactly per the approved plan caught a redundant local "Close" button duplicating the modal's shared footer Close button — invisible to both code review and automated tests because both would consider it "correct per plan." Fixed post-hoc as a deliberate UX improvement beyond the literal plan.

Reusable lessons:
1. **Playwright cannot stub `contextBridge`-exposed renderer APIs from `page.evaluate()`.** Assert on synchronous pre-IPC side effects of the real handler instead. For genuine main-process API mocking (dialogs, etc.), use `electronApp.evaluate(({dialog}) => {...})` via `_electron`, not `page.evaluate()`.
2. **`renderer.js` top-level IIFEs are one-directional scope boundaries.** A function declared inside a large IIFE is invisible to global-scope code elsewhere in the file. Before wiring cross-location calls, grep for `(function () {` / `})();` boundaries around the target line; bridge via `window._fnName = fnName;` immediately after definition if a cross-boundary call is needed.
3. **Merging UI surfaces that each carry independent per-scope state (per-event, per-batch) requires designing for isolation up front, not discovering it during implementation.** A first-pass plan that just "moves DOM around" is likely to accidentally conflate previously-independent scopes. Every function/ID/data-lookup reused across the merged copies must be parameterized (ID prefix AND the underlying data source, e.g. `eventPath`) — renaming only the ID prefix while leaving the data lookup unparameterized still acts on the wrong scope.
4. **A live screenshot pass after implementing UI restructuring work is not optional polish.** It catches a class of defect (visual redundancy, awkward juxtaposition of duplicated controls) that structural/behavioral tests and code review both miss by construction, because both only check conformance to the plan, not the rendered result.

Promote to agents (proposed):
- `ui-system-specialist.md` — lessons 1–4 (Playwright/contextBridge stubbing trap, cross-IIFE renderer.js scope trap, merged-UI-surface scope-isolation pattern, live-screenshot validation step). All four are renderer.js/UI-domain patterns with no other agent owning this surface; consolidated into one agent rather than duplicated.
- `metadata-specialist.md` — inspected, not updated. Task was pure UI restructuring with zero metadata/ExifTool/IPC logic changes; none of the lessons are metadata-domain rules.
