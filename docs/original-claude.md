...
- [x] Full emoji → inline SVG replacement — zero emoji in any rendered HTML. `const SVG` IIFE block in `renderer.js` (after `escapeHtml`) provides all icon strings (check, warn, skip, clock, loader, folder, camera, layers, save, sparkles, chevron, flag, info, play/pause, download, etc.) at consistent `viewBox="0 0 24 24"`, `stroke-width="1.6"`, `stroke-linecap/linejoin="round"`, `aria-hidden="true"`. `eventCreator.js` uses inline SVGs (it loads before renderer.js so cannot reference `SVG`). CSS `.dup-icon`, `.ob-hero`, `.ec-preview-label`, `.ec-evl-warn`, `.section-toggle`, `#importBtn` all changed from `font-size` + emoji to `display:flex; align-items:center` for SVG centering. Progress filename labels switched to `innerHTML` + `escapeHtml()` on all dynamic parts to allow SVG prefix icons.
- [x] Destination structure preview in Confirm Event Import modal — `#eiDestSection` / `#cmDestinationTree` inserted between Group Assignments and file count summary. `_renderDestinationTree(groups, eventData, photographerName)` in renderer.js builds an array-joined monospace tree that mirrors actual import routing: archive root folder → collection folder → event folder → [sub-event/] → photographer/ (N photos) → VIDEO/ (N videos). Single-component: photographer at level-3. Multi-component: sub-event at level-3, photographer at level-4. Unmapped groups render as `(Unassigned)/` rather than being silently skipped. Initial render shows `"Photographer (not selected)"` placeholder; tree updates live via `_updateTree(label)` wired to the photographer dropdown `onSelect`. Paths verified against importRouter: `collectionPath = coll._masterPath`, `eventFolderName = lastSeg(eventPath) = event.name` — identical to what the router uses. CSS: `var(--text-primary)` root/branch (bold scaffold), `var(--accent)` photographer nodes, `var(--text-muted)` muted/video/placeholder; hover expands truncated paths (`overflow:visible; white-space:normal`).
...
**SVG icon system** — `const SVG = (() => { ... })()` IIFE defined in renderer.js immediately after `escapeHtml`. Helper `i(path, w=14)` generates `<svg width=W height=W viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">`. Named exports: `check`, `checkCircle`, `warn`, `warnCircle`, `block`, `skip`, `clock`, `loader`, `download`, `pause`, `play`, `chevronRight`, `chevronDown`, `folder`, `folderLg`, `flag`, `info`, `camera`, `layers`, `save`, `sparkles`. **Critical:** `eventCreator.js` loads before `renderer.js` — it must inline SVG strings directly, cannot reference `SVG`.

**Confirm Event Import modal** — `showEventImportConfirmModal(groups, eventData)` renders `#eventImportModal`. Contains: event name, photographer TreeAutocomplete, group→sub-event mapping table (multi-component only), destination structure preview (`#eiDestSection` / `#cmDestinationTree`), file count summary, Cancel / Import actions. `_renderDestinationTree(groups, eventData, photographerName)` builds the preview tree (see Features Implemented for full spec).
