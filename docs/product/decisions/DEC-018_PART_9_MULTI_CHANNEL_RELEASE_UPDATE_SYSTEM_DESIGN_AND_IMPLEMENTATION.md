...
**2026-08-13 (later, same day) — real-client verification closed.** Real Windows hardware confirmed the isolation-mechanism decision above end-to-end: an existing v0.9.11 Stable install did not discover any RC; a manually-bootstrapped Preview install correctly auto-discovered, downloaded, and installed a newer RC (`v0.9.12-rc.2` → `v0.9.12-rc.3`) through the app's own updater; and switching back to Stable never downgraded the client. See [AI-FEAT-057](../features/AI-FEAT-057_MULTI_CHANNEL_RELEASE_UPDATE_SYSTEM.md)'s Acceptance Matrix for the full breakdown. This decision's scope is now fully evidenced at every tier (source, automated test, live CI, and real client) except a real future Stable release superseding a running RC, which is correctly out of scope for this pilot (would require publishing Stable).
