...
**Update (2026-08-14) — AI-FEAT-048's own scope is broader than this passage's original phrasing suggested.** A Knowledge Portal Stage 2 investigation, prompted by an explicit product-owner clarification about the Online Registry's intended purpose, forensically traced AI-FEAT-048's implementation end-to-end and found a substantially-implemented, VERIFIED (not merely intended) distributed-event-coordination mechanism: an operator's newly-created event is automatically registered with the relay server, persisted (`registry.json`), broadcast live to other connected devices and replayed to reconnecting ones, discoverable in a real "Online Registry" UI tab, and — critically — **adoptable**: a separated operator can generate a local `event.json` from the registry entry, preserving the exact event name/sequence/components, without a shared NAS connection. This is a materially larger scope than "a narrow cross-device validation guard... address parts of what distributed working requires" above implies — that phrasing was not wrong about what it described, but incomplete about AI-FEAT-048's full scope. The original passage above is preserved unedited per this system's append-only convention (§11 of `docs/product/CLAUDE.md`); this update does not rewrite it, only supplements it. The full evidence-classified account (20 forensic questions, each independently verified in source) lives in [AI-WF-006](workflows/AI-WF-006_SEE_WHO_ELSE_IS_ONLINE_AND_WHAT_THEYRE_WORKING_ON.md)'s "Event Discovery & Coordination" section — a deliberate scope decision: correcting AI-FEAT-048's own canonical record is a larger undertaking (a full evidence review of that entire feature) than this investigation's actual mandate (the Knowledge Portal's operator-facing model of it), and is recorded here as a known, disclosed gap in AI-FEAT-048's documentation rather than silently fixed in passing.
...
**Live-pilot confirmation (2026-08-13).** The isolation claim above was verified against real GitHub state, not only against installed-library source: a real `v0.9.12-rc.1`/`v0.9.12-rc.2` RC publication (triggered via `workflow_dispatch`) confirmed `/releases/latest` never stopped resolving to Stable's `v0.9.11` and Stable's `latest.yml`/`latest-mac.yml` remained byte-identical throughout — including during a real partial-publish failure on the Windows platform ([BUG-015](bugs/BUG-015_WINDOWS_RUNNER_DEFAULT_POWERSHELL_SHELL_MANGLES_ELECTRON_BUILDER_DOT_NOTATION_CLI_OVERRIDES.md), a Windows-runner default-shell/CLI-argument-parsing bug unrelated to the isolation mechanism itself, found and fixed the same day). This is the first real evidence that the isolation design holds under an actual failure condition, not only in the designed happy path.

**Real-client closure (2026-08-13, later the same day).** A third RC (`v0.9.12-rc.3`) and real Windows-hardware testing closed the one remaining gap: an actual v0.9.11 Stable installation did not discover any RC; a manual one-time install of rc.2 bootstrapped it into having the Update Channel selector at all (v0.9.11 predates Part 9 and has no such mechanism to bootstrap itself — a real, evidenced test-plan omission caught by a dedicated forensic audit, not an architecture defect); Preview correctly auto-discovered, downloaded, and installed a newer RC (rc.2 → rc.3) through the app's own updater; and switching back to Stable at every stage never triggered a downgrade to v0.9.11. See [AI-FEAT-057](features/AI-FEAT-057_MULTI_CHANNEL_RELEASE_UPDATE_SYSTEM.md)'s Acceptance Matrix for the complete evidence-tier breakdown — no claim in this feature's scope remains unverified except a real future Stable release superseding a running RC, which is correctly deferred pending an actual Stable publication.

### Knowledge & Onboarding — Ask AutoIngest

AI-RM-011, a parallel track like Multi-Channel Release Architecture above — not part of §3A–§3I's archive-capability sequence, since it documents the *operator's* access to knowledge about AutoIngest rather than the archive itself. Stage 1 (2026-08-13) proved a citation-honest, non-LLM answer engine was viable on top of the existing deterministic docs-tooling ranker ([DEC-019](decisions/DEC-019_KNOWLEDGE_ENGINE_REUSES_EXISTING_RETRIEVAL_NO_NEW_SEARCH_SYSTEM.md)); Stage 2 (2026-08-14) added Workflow records and an Intent/concept layer, merged to `main` ([DEC-020](decisions/DEC-020_STAGE_2_KNOWLEDGE_ARCHITECTURE_WORKFLOW_RECORDS_AND_CONCEPT_LAYER.md)).

A later, bench-only "Production Readiness Phase 1" prototyped a deterministic-retrieval improvement (BM25 + Reciprocal Rank Fusion over the existing lexical ranker), never committed, never wired into any production caller. A separate "Checkpoint 9–14" bench track (One-Brain Architecture qualification) prototyped a session-handle-based, five-operation deterministic knowledge/tool surface (`search_autoingest`/`read_autoingest`/`capability_status`/`roadmap_status`/`check_relationship`) against the "Candidate C Knowledge Model" (`lib/knowledgeModel/`, promoted inert in commit `2cc17d9`, predating the "Ask AutoIngest — Production Implementation" track below) — also never wired into production. The Product Owner then authorized **Stage 1** (2026-09-04) of a separate, gated **"Ask AutoIngest — Production Implementation" track** — deliberately not the same "Stage 1/Stage 2" numbering as the Knowledge Engine's own 2026-08-13/08-14 stages above; distinguish by date, not by stage number — to productionize the retrieval design behind tests, with **zero production callers** (`scripts/product-docs/lib/askRetrieval/`, [DEC-022](decisions/DEC-022_ASK_AUTOINGEST_STAGE_1_DETERMINISTIC_RETRIEVAL_PRODUCTIONIZATION.md)), then **Stage 2** (2026-09-04, same day) to productionize the deterministic knowledge base/tool foundation on top of it — auditing the qualified Checkpoint 9-14 bench code piece by piece (promote/reimplement/reuse/obsolete, never copied blindly), resolving the Decision-backed-knowledge and relationship-CONFLICT-detection design questions against real corpus data rather than assumption, and finding (and disclosing, not silently fixing) one genuine pre-existing Knowledge Model authoring defect in the process (`scripts/product-docs/lib/askKnowledge/`, [DEC-023](decisions/DEC-023_ASK_AUTOINGEST_STAGE_2_DECISION_BACKED_KNOWLEDGE_AND_RELATIONSHIP_CONFLICT_POLICY.md)) — still **zero production callers**. This is a controlled, explicitly-gated promotion from experimental retrieval/knowledge engineering toward an eventual production-integrated answer engine; each further step (the Qwen3.5-4B runtime, the conversational orchestrator, IPC, or the renderer — Stage 3 and beyond) requires its own separate Product Owner authorization and is not implied by either stage completed so far. As of this document's update, Ask AutoIngest remains not tester-ready and not integrated into any shipped surface.
