...
A later, bench-only "Production Readiness Phase 1" prototyped a deterministic-retrieval improvement (BM25 + Reciprocal Rank Fusion over the existing lexical ranker), never committed, never wired into any production caller. The Product Owner then authorized **Stage 1 of a separate, gated "Ask AutoIngest — Production Implementation" track** (2026-09-04) — deliberately not the same "Stage 1/Stage 2" numbering as the Knowledge Engine's own 2026-08-13/08-14 stages above; distinguish by date, not by stage number — to productionize that design behind tests, with **zero production callers** (`scripts/product-docs/lib/askRetrieval/`, [DEC-022](decisions/DEC-022_ASK_AUTOINGEST_STAGE_1_DETERMINISTIC_RETRIEVAL_PRODUCTIONIZATION.md)). This is the first concrete step of a controlled, explicitly-gated promotion from experimental retrieval engineering toward an eventual production-integrated answer engine; each further step (wiring the module into an actual caller, the Qwen runtime, IPC, or the renderer) requires its own separate Product Owner authorization and is not implied by this one. As of this document's update, Ask AutoIngest remains not tester-ready and not integrated into any shipped surface.
