...
### Multi-Event Import Architecture

Part of AI-RM-011 (a parallel ingestion-workflow track, not part of §3A–§3I's lettering). Recorded 2026-09-20, while implemented and verified on `feature/multi-event-import` and not yet merged — this section describes the architecture as built, not a released state.

Event Import had always assumed **one active event per import**: GroupManager was a module singleton reset whenever the event changed (a safety guarantee that group state could not leak across events), and the only import transaction, `import:commitTransaction`, is inherently one event (locks, an atomic event.json commit, rollback, metadata context). In practice one source often holds media for several events.

The architectural shift ([DEC-019](decisions/DEC-019_MULTI_EVENT_IMPORT_ARCHITECTURE.md), AI-FEAT-058):

- **Guarantee moved from "reset" to "isolation".** Group/component state still must never leak across events, but that is now guaranteed structurally — each participating event owns an isolated GroupManager instance inside an event workspace, with the old export kept as a facade bound to the Current Event. Persistent file→event ownership is derived from those stores and made exclusive by a claim hook. UI selection, persistent assignment, the Current Event and a file's Assigned Event became four separate concepts.
- **Transaction semantics were not widened.** Atomicity remains per event.json. A multi-event import is an *orchestration of independent event transactions* (one existing `import:commitTransaction` per event, run by a runner with an explicit failure policy: event-local failures continue, source-wide fatals stop). A session is therefore not filesystem-atomic and can partially complete; retries never overwrite. This is a change to what "one import" means, recorded here because it touches the meaning of atomic import (§3D), even though the per-event guarantee itself is unchanged.
- **Identity moved out of UI state.** Batch-scoped state (metadata progress attribution, Local First sync manifests, Deep Verify accumulation) is keyed by an identity carried in payloads instead of "whichever event is current" — a class of assumption that was invisible while only one event could ever be in flight.
- **Source-agnostic by construction.** The model, plan and runner never reference a source type; source-specific behavior (Eject) stays outside.
- **Extension point, not speculation.** The event workspace is the documented place for any future event-scoped state; nothing was added for a feature that is not shipping (Per-Photo Tag Refinement is held out of Stable, `ac75466`).

A transitional decision is recorded honestly: the original single-event handler remains for one-event, groups-only sessions, as a regression-safety measure, with the plan/runner designed so a single event is simply a one-event plan.
...
- **A "reset" guarantee can be replaced by an "isolation" guarantee when scope must be preserved, not discarded.** GroupManager's reset-on-event-change kept group state from leaking across events; Multi-Event Import needed several events' state alive at once, so the guarantee became structural (isolated per-event instances, exclusive ownership) rather than procedural (AI-FEAT-058, DEC-019). When generalising a single-context flow, audit for scalar globals and "current X" reads on asynchronous completion paths first — they are the assumptions that silently break.
...
| Multi-Event Import Architecture (parallel ingestion-workflow track, not §3-lettered) | AI-FEAT-058, AI-FEAT-017, AI-FEAT-019, AI-FEAT-021 | `docs/multi-event-import.md`, `docs/group-manager.md`, `docs/ingestion-flow.md` | [DEC-019](decisions/DEC-019_MULTI_EVENT_IMPORT_ARCHITECTURE.md) |
