...
**Part 8 note**: a full external discussion (as opposed to a single structured summary submitted through `memory import`) now has its own dedicated, richer record family and policy — see [18_ENGINEERING_CONVERSATION_POLICY.md](18_ENGINEERING_CONVERSATION_POLICY.md). That system may itself create or continue an `AI-MEM-####` capsule through this policy's own significance gate (§ 8) unchanged; it is an additional contribution path into this layer, not a replacement for it.
