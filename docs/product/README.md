...
`docs/product/conversations/` (governed by [18_ENGINEERING_CONVERSATION_POLICY.md](18_ENGINEERING_CONVERSATION_POLICY.md)) extends Part 6's Engineering Memory to discussions that happen **outside** an active local Claude Code session — ChatGPT, an external Claude conversation, Codex, Gemini, a human design-review meeting, an engineering email thread, or any future tool. A vendor-neutral **Engineering Conversation Packet** (ECP 1.0, `scripts/product-docs/schemas/engineering-conversation-packet.schema.json`) is the single handoff format any of those tools (or a human, by hand) can produce; `node scripts/product-docs/cli.js conversation import --format ecp --file <path>` (see [scripts/product-docs/automation/conversation/README.md](../../scripts/product-docs/automation/conversation/README.md)) takes it from there: redaction, schema validation, duplicate/continuation detection, evidence-based feature/roadmap/bug/decision/memory linkage (reusing Part 7's ownership engine and Part 7B's decision-intelligence pattern), and — only when the packet clears its own significance bar (§ 8 of the policy) — allocation of a permanent `ENG-CONV-####` record. `ENG-CONV`, not `AI-CONV`, because the record describes engineering discourse, which may originate from a human meeting with no AI involvement at all.

Same authority tier as Engineering Memory (historical evidence, never a runtime contract — § 3 of the policy); same tool-neutral, no-embeddings, no-network non-goals as every other Part in this system. Deliberately isolated from the live Evidence Packet auto-finalize pipeline (§ 10 of the policy) — an imported conversation's own decision/bug linkage never reaches a canonical record through a git hook, only through an explicit, foreground CLI action. See [conversations/README.md](conversations/README.md) for the day-to-day workflow and [conversations/CHATGPT_HANDOFF.md](conversations/CHATGPT_HANDOFF.md) for the concrete ChatGPT instruction.
