...
## Zero-Touch Automation (Part 7)

`scripts/product-docs/` (`node scripts/product-docs/cli.js {automation,release,context} <sub>` — see [scripts/product-docs/README.md](../../scripts/product-docs/README.md) § Part 7 and [CLAUDE.md](CLAUDE.md) § 20) extends Part 5's automation from "an agent should run these commands before declaring work complete" to "the project owner should never need to ask." The evidence discipline above is unchanged — Part 7 adds no new "when is this justified" rule of its own; `automation/decisionIntelligence.js`'s structural-signal detector reuses the exact same "≥2 alternatives + an accepted solution" bar `documentationPlanner.planDecisionRecords` already enforces, and `automation/ownershipEngine.js`'s multi-signal resolver never asserts an owner without citing the specific evidence (a Related Files citation, a subsystem directory, a require-graph edge, a test pairing, a decision/bug citation, or a git co-change ratio) behind it. What Part 7 changes is *when* these run: git hooks (version-controlled at `scripts/product-docs/hooks/`, installed into the real `.git/hooks/` only via an explicit, human-approved `automation install-hooks`) invoke the same finalize/validate machinery automatically at commit and push time, so a normal engineering session never requires a separate "now update the docs" step.

## Multi-AI Engineering Conversation Integration (Part 8)

`docs/product/conversations/` (`node scripts/product-docs/cli.js conversation <sub>`, see [18_ENGINEERING_CONVERSATION_POLICY.md](18_ENGINEERING_CONVERSATION_POLICY.md)) extends the Engineering Memory layer above to discussions that happen outside this repository's own sessions entirely. Same significance discipline: a conversation earns a canonical `ENG-CONV-####` only when its own imported content clears the policy's § 8 bar (an explicit requirement, an accepted/rejected approach, a bug discussion, an implementation request, or a substantive open question) — a schema-valid but engineering-content-free import is retained locally, never allocated a permanent ID. Same evidence discipline: a section with nothing the imported packet actually states renders "Evidence pending — not present in imported packet," never a plausible reconstruction. One addition beyond Part 6: because the source text originates entirely outside this repository's trust boundary, every import is redacted and schema-validated before any canonical write, and a conversation's own decision/bug linkage is deliberately excluded from the git-hook-triggered auto-finalize pipeline Part 7 (above) introduced — see the policy's § 10 for why.
