...
const { filterRecordsByRelevance, conceptualRetrieve } = require('../retrieval');
...
check('integration-readiness addition: a scoped record IS included when the technical gate fired, even if its question type is not declared, provided it has real technicalDetail', () => {
  const scoped = fixtureRecord({ id: 'KM-fixture-scoped', recordRole: 'scoped', scopedQuestionTypes: ['TROUBLESHOOTING'], technicalDetail: 'fixture technical detail', provenance: [{ claim: 'technical fixture', source: 'fixture.js:3', type: 'code', confidence: 'high' }] });
  const result = filterRecordsByRelevance([scoped], 'CAPABILITY', false, true);
  assert.deepEqual(result, [scoped]);
});

check('integration-readiness addition: a scoped record with no technicalDetail is still excluded even if the technical gate fired', () => {
  const scoped = fixtureRecord({ id: 'KM-fixture-scoped', recordRole: 'scoped', scopedQuestionTypes: ['TROUBLESHOOTING'], technicalDetail: null });
  const result = filterRecordsByRelevance([scoped], 'CAPABILITY', false, true);
  assert.deepEqual(result, []);
});
