...
const REGRESSION_CORPUS_V3 = [
  ...
  {
    id: 'RF-5.3-003', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: 'Decision 5 flagship (validated phrasing) -- canonical Feature anchor first, Memory admitted only as historical context',
    status: 'control', category: 'memory-context',
    question: "Why was the Online Registry's documentation revised?",
    rationale: "IMPLEMENTED at Phase 5.3: the literally-worded flagship phrasing from the brief (RF-5.3-001, preserved unchanged as a documented, disclosed, out-of-scope retrieval defect -- it collides with AI-FEAT-027 via generic keyword overlap, unrelated to Decision 5) never reaches AI-FEAT-048 at all, so it cannot exercise the real admission mechanism. This re-phrased, topically-equivalent question does: AI-FEAT-048 (Online Registry) is decided as the current canonical Feature anchor BEFORE any historical lookup runs (capabilityStatus AVAILABLE, from the Feature's own operatorStatus alone); AI-MEM-0004 is then admitted into the real sources[] because it is grounded (its own Scope table names AI-FEAT-048 as a Secondary feature ID), the question carries real historical intent, and the capsule carries genuine differentiating chronology (6 revisions). AI-FEAT-048 itself has no real 'Related architectural evolution sections' citation (confirmed empty, not fabricated), so no Architecture member is expected here.",
    requiredMemberIds: ['AI-FEAT-048', 'AI-MEM-0004'], expectedStatus: 'AVAILABLE',
    requiredHistoricalRole: { id: 'AI-MEM-0004', expectedRole: 'historical-context' },
  },
  {
    id: 'RF-5.3-004', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Decision 5's 'Decision-vs-Memory overlap' required case -- both roles stay distinct, neither duplicates the other",
    status: 'control', category: 'memory-context',
    question: 'Why was the Knowledge Engine architected to reuse existing retrieval instead of building a new search system?',
    rationale: "DEC-019 is admitted via the pre-existing, completely unmodified Phase 5.2 governance mechanism (classification-eligible EXPLANATION, direct citation, reciprocal -- AI-FEAT-058's own header cites DEC-019, and DEC-019's own header cites AI-FEAT-058 back). AI-MEM-0004 is independently admitted via the new Phase 5.3 mechanism (grounded via its own Scope table's Primary feature ID, real historical intent, genuine chronology). Both appear together in the same real sources[] with genuinely distinct authority roles -- Decision answers the accepted WHY, Memory answers the PROCESS/CHRONOLOGY of reaching it -- proving Phase 5.3 does not duplicate or interfere with Phase 5.2's own, separate mechanism.",
    requiredMemberIds: ['AI-FEAT-058', 'DEC-019', 'AI-MEM-0004'],
    requiredHistoricalRole: { id: 'AI-MEM-0004', expectedRole: 'historical-context' },
  },
  {
    id: 'RF-5.3-005', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Decision 5's 'current capability negative -- vocabulary overlaps Memory/Architecture strongly' required case",
    status: 'control', category: 'memory-context',
    question: 'What is the Metadata Writing Engine?',
    rationale: "AI-FEAT-029 carries TWO real, structured 'Related architectural evolution sections' citations (§3A, §3C) -- strong real grounding -- but this is a plain EXPLANATION/current-capability question with no historical intent. Proves grounding alone is never sufficient: both real Architecture sections are correctly excluded from sources[], not merely unlabeled.",
    requiredMemberIds: ['AI-FEAT-029'],
    forbiddenMemberIds: ['ARCH-a-established-adobe-bridge-workflow-pre-autoingest', 'ARCH-c-metadata-automation'],
    expectedStatus: 'AVAILABLE', expectedClassification: 'EXPLANATION',
  },
  {
    id: 'RF-5.3-006', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Decision 5's 'evidence-pending Memory' required case -- epistemic qualification preserved verbatim, never silently promoted",
    status: 'control', category: 'memory-context',
    question: 'How did the Metadata Audit and Repair tab evolve?',
    rationale: "AI-MEM-0001's own Identity table Evidence classification field literally states 'Reconstructed from repository evidence only... no original chat transcript was available' -- a real, qualified, less-grounded epistemic status. Verified preserved verbatim on the real sources[] entry, never rewritten as a confident/verified classification.",
    requiredMemberIds: ['AI-FEAT-034', 'AI-MEM-0001'],
    requiredEvidenceQualificationSubstring: { id: 'AI-MEM-0001', substring: 'Reconstructed from repository evidence only' },
  },
  {
    id: 'RF-5.3-007', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Decision 5's 'planned/current conflict -- current canonical Feature wins current-state claims' required case",
    status: 'control', category: 'memory-context',
    question: 'Why was Backup Update Scanning originally described as a narrow cross-device validation guard?',
    rationale: "Architectural Evolution §3E's original passage describes AI-FEAT-040's distributed-working role narrowly; §3E's own later append-only Update block discloses that a related feature's scope was later found broader than originally documented. Regardless of what the historical narrative says, this control proves capabilityStatus is always sourced from AI-FEAT-040's own current, real operatorStatus (AVAILABLE) -- structurally impossible for the admitted Architecture citation to override or contradict it, since sourcesForRecord() appends historical-context entries strictly after status is already decided.",
    requiredMemberIds: ['AI-FEAT-040', 'ARCH-e-transfer-and-distributed-working'], expectedStatus: 'AVAILABLE',
    requiredHistoricalRole: { id: 'ARCH-e-transfer-and-distributed-working', expectedRole: 'historical-context' },
  },
  {
    id: 'RF-5.3-008', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Decision 5/Decision-C's required no-current-anchor control -- retrieval/grounding correctness, diagnostic-only (materiality safety closure)",
    status: 'control', category: 'memory-context',
    question: 'What archival practice existed before Aljamea-tus-Saifiyah adopted AutoIngest?',
    rationale: "SUPERSEDED by the materiality safety investigation: retrieval strength alone was proven insufficient to justify PUBLIC admission (see RF-5.3-011's own account of the ARCH-e counter-example). This entry now asserts retrieval/grounding CORRECTNESS only, via the diagnostic seam (explainHistoricalContext()) -- never the real public answer. Body indexing (lib/searchIndex.js's architecture_section keywords now include each section's own canonical body text) makes §3A genuinely, deterministically retrievable and identifiable as the strong, untied, type-leading Architecture candidate for this exact question (raw score 400: aljamea/archival/existed/practice; §3H at 200, §3C/§3F at 100 are real but correctly not the leader). But per the Product Owner's materiality-safety closure, NO Architecture record is exposed in the real public answer merely because it retrieved strongly and uniquely -- retrieved != material != admitted. Primary (DEC-004) and capabilityStatus (AVAILABLE) are the pre-existing, unmodified governance-primary answer.",
    requiredMemberIds: ['DEC-004'], expectedStatus: 'AVAILABLE',
    requiredDiagnosticUnanchoredMemberIds: ['ARCH-a-established-adobe-bridge-workflow-pre-autoingest'],
    forbiddenDiagnosticUnanchoredMemberIds: ['ARCH-h-current-architectural-position', 'ARCH-c-metadata-automation', 'ARCH-f-specialized-archival-workflows'],
    requiredDiagnosticUnanchoredRole: { id: 'ARCH-a-established-adobe-bridge-workflow-pre-autoingest', expectedRole: 'Architectural Evolution' },
    expectedPublicUnanchoredAbsent: true,
  },
  {
    id: 'RF-5.3-009', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: "Structural safety guard -- the unanchored fallback cannot alter the already-resolved primary/status, and NO Architecture/Memory record is exposed in the real public answer regardless of what the diagnostic seam retrieves",
    status: 'control', category: 'memory-context',
    question: 'What archival practice existed before Aljamea-tus-Saifiyah adopted AutoIngest?',
    rationale: "Deliberately the SAME question as RF-5.3-008 (not a different one), asserting the materiality-safety-closure invariant directly against the REAL answer object: matchedCapabilities[0] is never an ARCH-*/AI-MEM-* record, the answer.unanchoredHistoricalContext key is structurally absent (never attached -- see lib/knowledgeEngine.js's reconcileUnknownEvidenceWording, which computes the fallback only to narrow unknownAnswer()'s wording when needed, never to expose candidate data publicly), and capabilityStatus/primary match RF-5.3-008's own values exactly. RF-5.3-008 asserts the diagnostic seam still finds real evidence (retrieval/grounding correctness); this entry asserts none of that evidence leaks into the public answer (admission safety). Both must hold simultaneously.",
    requiredMemberIds: ['DEC-004'], expectedStatus: 'AVAILABLE',
    forbiddenMemberIds: ['ARCH-a-established-adobe-bridge-workflow-pre-autoingest', 'ARCH-h-current-architectural-position', 'AI-MEM-0001', 'AI-MEM-0002', 'AI-MEM-0003', 'AI-MEM-0004'],
    expectedPublicUnanchoredAbsent: true,
  },
  {
    id: 'RF-5.3-010', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: 'Architecture ambiguity control -- richer indexing does not imply blanket admission (current-capability negative, re-verified post body-indexing)',
    status: 'control', category: 'memory-context',
    question: 'What is the Metadata Writing Engine?',
    rationale: "Re-verified after body indexing specifically (not merely inherited from RF-5.3-005): AI-FEAT-029 still carries its two real, Feature-anchored Architecture citations (§3A, §3C), now with much richer keyword surfaces than at RF-5.3-005's original authoring time -- confirms richer recall alone still never implies admission, since this question carries no historical intent at all (the historicalContextForFeature() gate this question actually reaches is a completely separate, Feature-anchored mechanism from the unanchored fallback tested by RF-5.3-008/009, and was not touched by the body-indexing or materiality-safety-closure changes).",
    requiredMemberIds: ['AI-FEAT-029'],
    forbiddenMemberIds: ['ARCH-a-established-adobe-bridge-workflow-pre-autoingest', 'ARCH-c-metadata-automation'],
    expectedStatus: 'AVAILABLE', expectedClassification: 'EXPLANATION',
  },
  {
    id: 'RF-5.3-011', targetPhase: '5.3', decisionRef: 'Decision 5 of 8', auditRef: 'RF-5.4-002 forensic disposition -- the exact case that PROVED unanchored Architecture materiality is unresolved. Preserved, not weakened -- it exposed a genuine defect.',
    status: 'control', category: 'memory-context',
    question: 'What does Transfer Export do and why was its locking kept process-local?',
    rationale: "FORENSIC FINDING (Phase 5.3 closure, body-indexing + materiality-safety investigation): this Phase-5.4-owned question resolves governance-primary (DEC-021, unaffected). §3E (Transfer and Distributed Working) retrieves strongly and uniquely (raw score 300, untied, matched tokens export/local/transfer) -- genuine, non-coincidental token overlap, §3E does discuss Transfer Export's own origin. But investigated at the sentence level: the 3 matched tokens scatter across 6 different sentences (max concentration 2/3 in one sentence), and critically the 'local' match lands in an UNRELATED sentence ('Photographer-level lock handling (AI-FEAT-045) and Local-First background archive sync (AI-FEAT-044)') -- a coincidental match on 'Local-First', not 'process-local locking'. §3E never discusses the SPECIFIC process-local-lock decision DEC-021 forensically documents. THIS IS THE EXACT CASE that proved retrieval strength (eligibility) is insufficient for materiality -- no existing deterministic signal (title/body split, Feature-citation grounding, governance-to-Architecture relationship path, whole-record keyword overlap) distinguished it from the genuine §3A positive case; see DEC-020's own Post-Decision Evolution entry for the full empirical account across all five tested signals. Per the Product Owner's materiality-safety closure: §3E remains retrievable via the diagnostic seam (proving the retrieval/grounding machinery itself still works) but is NEVER exposed in the real public answer. RF-5.4-002's own real, previously-asserted primary/status/sources remain completely unaffected -- confirmed unchanged, never modified or weakened to accommodate this finding.",
    requiredMemberIds: ['DEC-021'], expectedStatus: 'AVAILABLE',
    requiredDiagnosticUnanchoredMemberIds: ['ARCH-e-transfer-and-distributed-working'],
    expectedPublicUnanchoredAbsent: true,
  },
