...
// ASK AUTOINGEST — CHECKPOINT 14. EXPERIMENTAL. Plain-Node CLI runner.
// Identical to runConversationsC13.js except it loads engineC14.js.
//
// Env:
...
//   ORCH_CONTEXT_SIZE optional integer, forwarded to createContext (Phase
//                      13's contextSize=16384 qualification). Omit for the
//                      engine's default.
...
const { loadEngine, createConversation } = require('./enginePR1');
...
async function main() {
  ...
  const contextSize = process.env.ORCH_CONTEXT_SIZE ? Number(process.env.ORCH_CONTEXT_SIZE) : undefined;
  ...
  console.log('[orchestrator-pr1] building knowledge context...');
  const { built } = build.assemble();
  const ctx = buildEngineContext(built);

  console.log(`[orchestrator-pr1] loading model ${modelPath}...`);
  let modelLoadCount = 0;
  const t0Load = Date.now();
  const engine = await loadEngine({ modelPath });
  const loadMs = Date.now() - t0Load;
  modelLoadCount += 1;
  console.log(`[orchestrator-pr1] MODEL_LOAD_COUNT=${modelLoadCount} (must be exactly 1), loadMs=${loadMs}${contextSize ? `, contextSize=${contextSize}` : ''}`);

  const results = [];
  for (const conv of conversations) {
    const outId = repeatIndex ? `${conv.id}__rep${repeatIndex}` : conv.id;
    console.log(`[orchestrator-pr1] === ${outId} ===`);
    const session = await createConversation(engine, ctx, built, contextSize ? { contextSize } : undefined);
    const transcript = [];
  ...
  console.log(`[orchestrator-pr1] wrote ${outPath}`);
  process.exit(0);
}

main().catch((err) => {
  console.error('[orchestrator-pr1] FATAL', err);
  process.exit(1);
});
