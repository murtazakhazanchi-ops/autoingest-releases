...
const { loadEngine, createConversation } = require('./engineC14');
...
async function main() {
  ...
  console.log('[orchestrator-c14] building knowledge context...');
  const { built } = build.assemble();
  const ctx = buildEngineContext(built);

  console.log(`[orchestrator-c14] loading model ${modelPath}...`);
  let modelLoadCount = 0;
  const t0Load = Date.now();
  const engine = await loadEngine({ modelPath });
  const loadMs = Date.now() - t0Load;
  modelLoadCount += 1;
  console.log(`[orchestrator-c14] MODEL_LOAD_COUNT=${modelLoadCount} (must be exactly 1), loadMs=${loadMs}${contextSize ? `, contextSize=${contextSize}` : ''}`);

  const results = [];
  for (const conv of conversations) {
    const outId = repeatIndex ? `${conv.id}__rep${repeatIndex}` : conv.id;
    console.log(`[orchestrator-c14] === ${outId} ===`);
  ...
  console.log(`[orchestrator-c14] wrote ${outPath}`);
  process.exit(0);
}

main().catch((err) => {
  console.error('[orchestrator-c14] FATAL', err);
  process.exit(1);
});
